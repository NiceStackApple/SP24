...
import { Clock, Sun, Moon, Eye, Heart, Skull, Loader, Flame, Biohazard, ShieldAlert, Ghost, AlertTriangle, LogOut, Trophy, Settings, X } from 'lucide-react';
  ...
  return (
    <div className="fixed top-1/3 left-0 right-0 z-[10000] flex flex-col items-center pointer-events-none animate-in fade-in slide-in-from-top-4 duration-500">
      <div className={`w-full max-w-4xl bg-black border-y-2 border-gray-900 py-6 flex items-center justify-center shadow-2xl ${glowColor}`}>
        <h1 className={`text-6xl md:text-8xl font-black font-mono tracking-[0.15em] uppercase ${textColor} ${textGlow}`}>
          {title}
        </h1>
      </div>
      <div className="mt-4 bg-black border border-gray-800 px-10 py-2 shadow-xl">
        <span className={`text-xl font-bold font-mono tracking-[0.3em] uppercase ${textColor}`}>
          {subtitle}
        </span>
      </div>
    </div>
      ...
      <div className="flex-[3] flex flex-col border-r border-gray-800 h-full relative">
         ...
         <header className="h-16 bg-black border-b border-gray-800 flex items-center justify-between px-6 shrink-0 z-20">
            <div className="flex items-center space-x-6">
              ...
              <div className="flex items-center space-x-3">
                ...
                <span className={`text-2xl font-mono font-bold ${isDay ? 'text-yellow-100' : 'text-blue-100'}`}>DAY {state.day}</span>
                {isVolcanoDay && <div className="ml-2 flex items-center gap-1 bg-red-900/50 border border-red-500 px-2 py-0.5 rounded animate-pulse"><Flame size={12} className="text-red-500" /><span className="text-xs font-bold text-red-500 font-mono">ERUPTION IMMINENT</span></div>}
                {isGasDay && <div className="ml-2 flex items-center gap-1 bg-green-900/50 border border-green-500 px-2 py-0.5 rounded animate-pulse"><Biohazard size={12} className="text-green-500" /><span className="text-xs font-bold text-green-500 font-mono">POISON GAS</span></div>}
                {isZoneDay && <div className="ml-2 flex items-center gap-1 bg-red-900/80 border border-red-600 px-2 py-0.5 rounded animate-pulse"><ShieldAlert size={12} className="text-white" /><span className="text-xs font-bold text-white font-mono">ZONE SHRINK</span></div>}
                {isMonsterDay && <div className="ml-2 flex items-center gap-1 bg-purple-900/50 border border-purple-500 px-2 py-0.5 rounded animate-pulse"><Ghost size={12} className="text-purple-500" /><span className="text-xs font-bold text-purple-500 font-mono">MONSTER HUNT</span></div>}
              </div>
              <div className={`flex items-center space-x-2 px-3 py-1 rounded-full font-mono font-bold text-sm border ${isDay && state.timeLeft <= 10 ? 'bg-red-900/30 border-red-900 text-red-500 animate-pulse' : 'bg-gray-900 border-gray-800 text-gray-400'}`}>
            ...
            <div className="flex items-center space-x-4">
               ...
               <div className="flex items-center space-x-2"><span className="text-xs text-gray-500 uppercase tracking-widest">Alive</span><span className="text-3xl font-mono font-bold text-white">{aliveCount}<span className="text-gray-600 text-lg">/24</span></span></div>
            </div>
         </header>
         ...
         <div className={`h-44 bg-black border-t border-gray-800 flex shrink-0 z-20 shadow-[0_-5px_20px_rgba(0,0,0,0.5)] transition-opacity duration-500 ${(state.volcanoEventActive || state.gasEventActive || state.monsterEventActive || state.zoneShrinkActive) ? 'opacity-20 pointer-events-none grayscale' : 'opacity-100'}`}>
            <div className="w-64 p-4 border-r border-gray-800 flex flex-col justify-center gap-3 bg-gray-900/30">
               {me ? (
                 <>
                   <div className="flex items-end justify-between border-b border-gray-800 pb-2"><span className="text-xs font-mono text-gray-500 uppercase">Integrity</span><div className="flex items-center gap-2 text-2xl font-bold text-white"><Heart size={20} className="text-red-500" />{me.hp}</div></div>
                   <div className="space-y-2">
                      <div className="space-y-1"><div className="flex justify-between text-[10px] text-gray-400 font-mono uppercase"><span>Hunger</span><span>{me.hunger}</span></div><div className="h-1.5 bg-gray-800 rounded-full overflow-hidden"><div className={`h-full ${me.hunger < 30 ? 'bg-red-500 animate-pulse' : 'bg-orange-500'} transition-all`} style={{ width: `${me.hunger}%` }}></div></div></div>
                      <div className="space-y-1"><div className="flex justify-between text-[10px] text-gray-400 font-mono uppercase"><span>Fatigue</span><span>{me.fatigue}</span></div><div className="h-1.5 bg-gray-800 rounded-full overflow-hidden"><div className={`h-full ${me.fatigue < 30 ? 'bg-red-500 animate-pulse' : 'bg-blue-500'} transition-all`} style={{ width: `${me.fatigue}%` }}></div></div></div>
                   </div>
                 </>
               ) : (
